...
parser = argparse.ArgumentParser(description = 'Script to run Distance in eXtremeDB')
...
for lines in args.lines:
	for columns in args.columns:
		...
		dim_format = '{{"name": "{0}{1}", "type": "double"}}'
		dim_types = dim_format.format("a", 0)
		for i in range(1, columns):
			dim_types = dim_types + ",\n          " + dim_format.format("a", i)
		for i in range(columns):
			dim_types = dim_types + ",\n          " + dim_format.format("b", i)
		...
		for i in range(2 * lines):
			lines_read.append( f.readline() )
		for i in range(lines):
			values_a = lines_read[i].split(" ")
			currentTime = get_datetime(values_a[0])[0]
			datapoint = { "time": currentTime.isoformat() + ".000Z" }
			for j in range(columns):
				datapoint["a" + str(j)] = float(values_a[j + 1])
			values_b = lines_read[i + lines].split(" ")
			for j in range(columns):
				datapoint["b" + str(j)] = float(values_b[j + 1])
