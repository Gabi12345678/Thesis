...
for lines in args.lines:
	for columns in args.columns:
		...
		generate_from_template(args.template_import_config, args.import_config, 
					[("<base_dir>", args.base_dir), ("<dim_types>", dim_types), ("<datasource>", "d1")])
		...
		dim_types = ',\n	  '.join(dim_types)
		generate_from_template(args.template_import_config, args.import_config,
					 [("<base_dir>", args.base_dir), ("<dim_types>", dim_types), ("<datasource>", "d2")])
		...
		curs = conn.cursor()
		print("*" * 100)
		curs.execute("SELECT * FROM d2")
		for row in curs:
			print(row)
		curs = conn.cursor()
		print("*" * 100)
		curs.execute("SELECT d1.dim0 as a0, d2.dim0 as b0 FROM d1, d2")
		#curs.execute("EXPLAIN PLAN FOR SELECT d1.dim0 as a0, d2.dim0 as b0 FROM d1, d2")
		for row in curs:
			print(row)
