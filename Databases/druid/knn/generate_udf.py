...
parser = argparse.ArgumentParser(description = 'Script to run KNN in Druid')
...
for lines in args.lines:
	for columns in args.columns:
		...
		unlabelDims = [ "dim" + str(i) for i in range(columns) ]
		labelDims = ["l" + str(i) for i in range(columns) ]
		dim_types = [ "{ \"name\": \"dim" + str(i) + "\", \"type\": \"double\" }" for i in range(columns)] + \
			    [ "{ \"name\": \"l" + str(i) + "\", \"type\": \"double\" }" for i in range(columns)  ] + \
		...
		for i in range(lines):
			lines_read[i] = f.readline()
		for i in range(lines):
			lines_read[i + lines] = f.readline()
		for i in range(lines):
			valuesUnlabel = lines_read[i].split(" ")
			valuesLabel = lines_read[i + lines].split(" ")
			currentTime = get_datetime(valuesUnlabel[0])[0]
			datapoint = { "time": currentTime.isoformat() + ".000Z" }
			for j in range(columns):
				datapoint["dim" + str(j)] = float(valuesUnlabel[j + 1])
				datapoint["l" + str(j)] = float(valuesLabel[j + 1])
			datapoint["label"] = float(random.randrange(5))
		...
		ts = query.timeseries(datasource = 'master', 
				...
				aggregations = { "knnresult": {"type": "knn", "timeField": "__time", "unlabelColumns": unlabelDims, "labelColumns": labelDims, "label": "label"}})
		...
		print("CD time:", final_time_udf - initial_time_udf)
		print("*" * 100)
