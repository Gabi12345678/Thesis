...
char sample_descr[] = "Sample 'collate' demonstrates custom collations for a tree index.\n";
const char * db_name = "collate";
char * fruits[] = {
...
int main(int argc, char* argv[])
{
  ...
  /* Open database */
  rc = sample_open_database( db_name, colldb_get_dictionary(), DATABASE_SIZE, CACHE_SIZE, 
  ...
  if ( MCO_S_OK == rc ) {
    ...
    if ( MCO_S_OK == rc ) {
      ...
      if ( MCO_S_OK == rc ) {
        ...
        rc = mco_trans_start(db, MCO_READ_ONLY, MCO_TRANS_FOREGROUND, &t);
        if (rc == MCO_S_OK) {
          /* Using custom collate tree index */
      ...
      if ( MCO_S_OK == rc ) {
        ...
        if ( MCO_S_OK == rc ) {
          ...
          rc = Record_tcoll_find(t, search, (uint2)strlen(search), &rec);
          if ( MCO_S_OK == rc ) {
            Record_name_get(&rec, buf, 11, &len);
            Record_value_get(&rec, &value);
            printf("\n\tCustom (case-insensitive): Found '%s', value = %d", buf, value);
          } else {
            printf("\n\tCustom (case-insensitive) - '%s' not found", search );
          }
          ...
          if ( MCO_S_OK == rc ) {
            Record_name_get(&rec, buf, 11, &len);
            Record_value_get(&rec, &value);
            printf("\n\tStandard (case-sensitive): Found '%s', value = %d\n", buf, value);
          } else {
            printf("\n\tStandard (case-sensitive) - '%s' not found\n", search );
          }
        }
        rc = mco_trans_commit(t);
      }
