 ...
 ****************************************************************/
#include <common.h>
#include "index_join_db.h"

char sample_descr[] = "Sample 'index_join' demonstrates using an index to join two classes.\n";
const char * db_name = "index_join_db";
...
struct department_data departments[] = {
    { "Accounting", "Acct", 101 }, { "Engineering", "Eng", 102 }, { "Customer Service", "CS", 103 }, { "Technical Support", "TS", 104 }
};
...
MCO_RET fill_database(mco_db_h db)
{
    ...
    for (i = 0; i < sizeof(departments) / sizeof(departments[0]) && MCO_S_OK == rc; ++i) {
        ...
        if (MCO_S_OK == rc)  {
            ...
            printf("\n\t%d) %s, %s, dept_no=%d", i, departments[i].code, departments[i].name, departments[i].dept_no );
    ...
    for (i = 0; i < sizeof(employee_departments) / sizeof(employee_departments[0]) && MCO_S_OK == rc; ++i) {
        ...
        if (MCO_S_OK == rc) {
            ...
            Department dept;
            rc = Department_Icode_find(t, employee_departments[i].dept_code, (uint2)strlen(employee_departments[i].dept_code), &dept);
            if (MCO_S_OK == rc)
            {
                uint2 dept_no = 0;
                ...
                if (MCO_S_OK == rc)
                {
                    ...
                    printf("\nUnable to extract dept_no for Department code = %s", employee_departments[i].dept_code);
                    rc = mco_trans_rollback(t);
                }
            } else {
                printf("\nUnable to find Department Code %s", employee_departments[i].dept_code);
                rc = mco_trans_rollback(t);
            }
        }
    }
    ...
    if (MCO_S_OK == rc) {
        ...
        if (MCO_S_OK == rc) {
            ...
            if (MCO_S_OK == rc) {
                ...
                // 3. Position the cursor in the Idept_name index to the first object with this dept_no
                ...
                while (MCO_S_OK == rc) {
                  ...
                  // If the two names are not equal, display the name (if dept_no is still the same)
                  if (0 != strcmp(name, search_name)) {
                    // Verify that the dept_no is still the same, otherwise break out of loop
...
int main(int argc, char* argv[])
{
    ...
    rc = sample_open_database(db_name, index_join_db_get_dictionary(), DATABASE_SIZE, CACHE_SIZE,
