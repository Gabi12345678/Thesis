...
#else
const uint4 database_size = (64 * 1024 * 1024 * MEMORY_SIZE_X64_MULT + MEMORY_SIZE_WCHAR_ADD);
uint4 n_records = 20000;
#endif
...
int main(int argc, char* argv[])
{
    ...
    insert(db, n_records, 1, random_seed, random_sequence);
    search(db, n_records, random_seed, random_sequence);
    scan(db, n_records, n_traversals);
    update(db, n_records, 1, random_seed, random_sequence, n_updates);
    delete(db, n_records, 1, random_seed, random_sequence);
