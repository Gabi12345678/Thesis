...
const char db_name[] = "iotrtdb";
#define ROUTER_DATABASE_SIZE    16*1024*1024
...
void PrintConfig(mco_db_h db)
{
    ...
    if (rc == MCO_S_OK) {
        printf("\n%10s %10s %10s %20s\n-----------------------------------------------------\n",
               "SrcAgentID", "DstAgentID", "ParamID", "Value");
        for (rc = mco_cursor_first(t, &csr); rc == MCO_S_OK; rc = mco_cursor_next(t, &csr)) {
            ...
            printf("%10llu %10llu %10u %20f\n", cobj.src_agent_id, cobj.dst_agent_id, cobj.param_id, cobj.value);
        }
        printf("\n");
    }
...
void PrintSensors(mco_db_h db)
{
    ...
    if (rc == MCO_S_OK) {
        printf("\n%10s %10s %10s %10s %10s\n------------------------------------------------------\n",
               "SrcAgentID", "DstAgentID", "SensorID", "Timestamp", "Value");
        for (rc = mco_cursor_first(t, &csr); rc == MCO_S_OK; rc = mco_cursor_next(t, &csr)) {
            Sensor_fixed cobj;
            CHECK(Sensor_from_cursor(t, &csr, &obj));
            CHECK(Sensor_fixed_get(&obj, &cobj));
            printf("%10llu %10llu %10d %10llu %10f\n", cobj.src_agent_id, cobj.dst_agent_id, cobj.sensorId, cobj.ts, cobj.value);
            ++n_recs;
        }
    }
...
int main(int argc, char *argv[])
{
    ...
    CHECK(mco_db_open_dev(db_name, iotrouter_get_dictionary(), &dev, 1, &db_params));
    ...
    printf("Start listening on the port 15001\n");
    CHECK(mco_iot_comm_listen(comm, "15001", 0));
    CHECK(mco_iot_replicator_connect(repl, conn_string, 2*1000, 0));

    sample_pause("Press Enter to stop the router...");
