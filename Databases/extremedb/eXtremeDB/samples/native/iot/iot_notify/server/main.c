...
{
    printf("ACK  received from agent %llu : %llu[%llu]->%llu, ts=%llu, errcode=%d (%s)\n", mco_iot_conn_get_agent_id(iotc),
           ack->sender_agent_id, ack->sender_db_id, ack->receiver_agent_id, ack->timestamp, ack->error_code, mco_strerror(ack->error_code));
    return IOT_CALLBACK_OK;
}
