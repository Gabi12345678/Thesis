...
bool parse_cmd(int argc, char *argv[])
{
    ...
    for (int i = 1; i < argc;)
    {
        ...
        if (*p++ == (char)'-')
        {
            ...
            else if (strcmp(p, "freeze") == 0)
            {
                freeze_on_exit = 1;
            }
			else if (strcmp(p, "simple") == 0)
			{
				skip_some_updates = true;
			}
			else
			{
                return false;
...
# ifdef _AIX
        params.diskCacheMemoryAddress = (char*) DATABASE_MAP_ADDRESS + 0x10000000;
# endif
        engine->open(params);
...
int main(int argc, char *argv[])
{
    ...
    {
        printf("usage: tpc [-init] [-simple] [-tpc n] [-clients n]\n");
        printf("\t-init\tinitialize the tables\n");
        printf("\t-simple\tskip some updates\n");
