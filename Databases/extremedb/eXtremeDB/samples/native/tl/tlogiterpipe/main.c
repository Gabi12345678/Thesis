...
char sample_descr[] = {
  "Sample 'tlogiterpipedb' demonstrates the use of transaction logging \niteration functionality. "
  "This sample transfers transactions between \ntwo databases inside one process through a pipe.\n"
  "The pipe works with blocking (with no extension files).\n"
};
const char *db_name = "tlogiterpipedb";
const char *db_name2 = "tlogiterpipedb2";
...
MCO_RET open_database (const char *db_name, void *mem_ptr, mco_size_t mem_sz, void *pipe_ptr, mco_size_t pipe_sz)
{
  ...
  return mco_db_open_dev (db_name, tlogiterpipedb_get_dictionary (), dev, n_dev, &db_params);
}
...
void IteratorThread( sample_task_t * descriptor )
{
  ...
  rc = open_database (db_name2, mem_ptr, DATABASE_SIZE, 0, 0);
...
MCO_RET use_database( mco_db_h con )
{
  ...
  if ( MCO_S_OK == rc )
  {
    printf( "\n\tDatabase has been cleaned", i);
  }

  /* Add 100 new keys again */
  rc = mco_trans_start( con, MCO_READ_WRITE, MCO_TRANS_FOREGROUND, &t );
  if ( MCO_S_OK != rc )
      return rc;
