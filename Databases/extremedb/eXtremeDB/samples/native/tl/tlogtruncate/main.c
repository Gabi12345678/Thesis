...
#include <stdio.h>
#include <common.h>
#include <tlogtruncatedb.h>
#include "mcolog.h"

char sample_descr[] = {
  "Sample 'tlogtruncatedb' demonstrates the use of log truncation.\n"
};
const char *db_name = "tlogtruncatedb";
const char *tl_log  = "tlogtruncate_tl.log";
...
int main(int argc, char** argv) {
  ...
  rc = sample_open_database( db_name, tlogtruncatedb_get_dictionary(), DATABASE_SIZE, CACHE_SIZE, 
  ...
  {    
    ...
    if ( MCO_S_OK == rc )
    {
      ...
      while ( 5 > g_size_warn_count )
      {
        ...
        /* Save database checkpoint and truncate log if size of log has exceeded limit*/
        if ( g_size_warn_flag == MCO_YES )
        {
          ...
          /* Truncate log */
          rc = mco_translog_truncate( connection );
          sample_rc_check( "\tTruncate log", rc );
          printf( "\n" );
