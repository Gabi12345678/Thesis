...
SELECT DISTINCT S.sname FROM S WHERE S.sid NOT IN (SELECT SP.sid FROM SP WHERE SP.pid = 'P2');

SELECT sid, sname FROM S WHERE status IN (8*2, 64/2 - 2);

SELECT * FROM S WHERE sid IN (
      SELECT sid FROM SP WHERE qty > 700 UNION SELECT sid FROM SP WHERE qty < 500 );
