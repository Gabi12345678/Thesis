...
def init_runtime(disk=False, tmgr='mursiw', shm=False,
                 ...
                 UsePosixLibraries=False, UsePerfmon=False, sql_workspace_limit=None, skip_load=False):
    """
    ...
    if sys.platform in ('win32',):
        ...
        if runtime_path is not None:
            rt_path.append(runtime_path)

        pythonpath = os.path.dirname(__file__)
        bin_so_path = os.path.abspath(os.path.join(pythonpath, "..")) + ".so"

        rt_path.append(bin_so_path)
        runtime_path = os.path.sep.join(rt_path)

        modname = ['eXtremeDB', 'eXtremeDB_debug'][debug]

        if not sync_library or sync_library is True:
            sync_library = 'mcosw32.lib'
        if not memory_library:
            memory_library = ['mcomw32', 'mcomconv'][shm]

        # Add bin.so to OS path
        os_path = os.environ['PATH']
        os_path += os.path.sep + bin_so_path
        os.environ['PATH'] = os_path

        import ctypes
        res = ctypes.windll.kernel32.SetDllDirectoryA(bin_so_path)

        if res == 0:
            raise Exception("SetDllDirectory failed")
    else:
        if not sync_library or sync_library is True:
        ...
        if debug:
            modname = 'eXtremeDB_debug'
        else:
            modname = 'eXtremeDB'
        if not skip_load:
            import eXtremeDB_load
    ...
    _eXtremeDB.runtime_start(globals(), disk, tmgr.lower()=='mvcc', shm, sync_library, memory_library, runtime_path,
                             cluster, ha, iot, sql_workspace_limit)

    global _convert_py2value
    _convert_py2value = _eXtremeDB._convert_py2value

    global _convert_value2py
    _convert_value2py = _eXtremeDB._convert_value2py
    ...
    if "MasterConnection" in dir(_eXtremeDB):
        setattr(sys.modules[__name__], "MasterConnection", _eXtremeDB.MasterConnection)

    if "ReplicaConnection" in dir(_eXtremeDB):
        setattr(sys.modules[__name__], "ReplicaConnection", _eXtremeDB.ReplicaConnection)

    if "SequenceIterator" in dir(_eXtremeDB):
        setattr(sys.modules[__name__], "SequenceIterator", _eXtremeDB.SequenceIterator)

    if "SqlEngine" in dir(_eXtremeDB):
        setattr(sys.modules[__name__], "SqlEngine", _eXtremeDB.SqlEngine)
        setattr(sys.modules[__name__], "SqlServer", _eXtremeDB.SqlServer)
        setattr(sys.modules[__name__], "SqlAggregator", _eXtremeDB.SqlAggregator)
        setattr(sys.modules[__name__], "AggregatedConnection", _eXtremeDB.AggregatedConnection)
        setattr(sys.modules[__name__], "SqlMultiResult", _eXtremeDB.SqlMultiResult)
        setattr(sys.modules[__name__], "AsyncConnection", _eXtremeDB.AsyncConnection)

    if "IoTCommunicator" in dir(_eXtremeDB):
        setattr(sys.modules[__name__], "IoTCommunicator", _eXtremeDB.IoTCommunicator)

    if "IoTReplicator" in dir(_eXtremeDB):
        setattr(sys.modules[__name__], "IoTReplicator", _eXtremeDB.IoTReplicator)
...
def connect(dbname = None, port = 0, nodes = None, nReplicas = 1, replType = ReplicationType.SQL_REPLICATION,
            ...
            sql_login=None, sql_password=None, ssl_params=None, async=False, nWorkers = 2, compressionLevel = 0
):
    ''' Connect to existing in database by name
          To connect to shared memory database use only name parameter
          To connect to RSQL database and use DB API SQL protocol, use host and port parameters
          To connect to Distributed SQL database use named parameters nodes and maxConnectAttempts,
    ...
    if not nodes is None:
        conn = _eXtremeDB.connect(nodes = nodes, nReplicas=nReplicas,
                                  ...
                                  async=async, nworkers=nWorkers, compressionLevel = compressionLevel)
    else:
        conn = _eXtremeDB.connect(dbname, port, nodes, nReplicas, maxConnectAttempts,
                                  useConnectionPool=useConnectionPool, txBufSize=txBufSize, localDomain=local,
                                  connectTimeout=2000L, readTimeout=1200*1000L,
                                  sql_login=sql_login, sql_password=sql_password, ssl_params=ssl_params,
                                  async=async, nworkers=nWorkers, compressionLevel = compressionLevel)
...
def load_dictionary(schema, nosort=True, dumpxml=False, genhpp=False, gencs=False, genjava=False, outDir='.', csNamespace='eXtremeDB',
                    javaPackage='eXtremeDB', cmode1=False, genXmlMethods=False, genJsonMethods=False, genSql=False, largeDatabase=False, wcharSize=2,
                    use_prefix=False, include_dir='.', compact=False, persistent=False, transient=False, suppress_api=False, debug=False):
    global _eXtremeDB
...
def register_function(rettype, name, func, n_args):
    global _eXtremeDB
    _eXtremeDB.register_function(rettype, name, func, n_args)
...
def open_database(dbname, dictionary, is_disk = False,
    ...
    """
    db = _eXtremeDB.open_database(dbname=dbname, dictionary=dictionary, is_disk=is_disk,
                                  ...
                                  databaseSnapshotFilePath=databaseSnapshotFilePath,
                                  ...
                                  backup_max_passes = backup_max_passes, backup_map_filename = backup_map_filename, cipherKey=cipherKey,
                                  compressionLevel = compressionLevel, compressionMask = compressionMask, expectedCompressionRatio = expectedCompressionRatio,
...
def _attach_sql_session(session):
    return _eXtremeDB._attach_sql_session(session)
...
class LogParams(object):
    ...
    def __init__(self, RedoLogLimit = None, DelayedCommitThreshold = None,
        ...
        self.DefaultCommitPolicy =Policy = DefaultCommitPolicy
...
class ClusterTCPParams(ClusterNWParams):
    def __init__(self, connectTimeout=5000, connectInterval=200, socketSendBuf=0, socketRecvBuf=0,
		         socketDomain=McoSockDomain.InetDomain, keepAliveTime=1000, keepAliveProbes = 10, ssl_params = None,
                 compressionLevel = 0):
...
class HANotifications(object):
    ...
    MCO_REPL_NOTIFY_CONNECT_FAILED = 1 # "connect failed" notification 
    MCO_REPL_NOTIFY_DB_EQUAL       = 2 # "no need to load DB" notification 
    MCO_REPL_NOTIFY_DB_LOAD_BEGIN  = 3 # "begin loading DB" notification 
    MCO_REPL_NOTIFY_DB_LOAD_FAILED = 4 # "loading failed" notification, "param1" of notification callback contains MCO_RET code 
    MCO_REPL_NOTIFY_DB_LOAD_OK     = 5 # "succesful loading" notification, "param1" of notification callback contains MCO_E_HA_REPLICA_STOP_REASON code
    MCO_REPL_NOTIFY_COMMIT_FAILED  = 6 # "commit failed" notification, "param1" of notification callback contains MCO_RET code
    MCO_REPL_NOTIFY_REPLICA_STOPPED= 7 # "stopped" notification, 
    MCO_REPL_NOTIFY_DB_CREATION_FAILED = 8 # "database creation failed" notification, "param1" of notification callback contains MCO_RET code 
    MCO_REPL_NOTIFY_HOTSYNC            = 9 # "begining of hot synchronization" notification 
    MCO_REPL_NOTIFY_EOHOTSYNC          = 10 # "end of hot synchronization" notification
    MCO_REPL_NOTIFY_STATEFUL_SYNC      = 11 # "begining of stateful replication" notification 
    MCO_REPL_NOTIFY_STATEFUL_SYNC_END  = 12 # "end of stateful replication" notification 
    MCO_REPL_NOTIFY_MASTER_DB_EXTENDED = 13 #  master database was extended by mco_db_extend_dev() call 
    MCO_REPL_NOTIFY_MASTER_DB_CLEANED  = 14 #  master database was cleared by mco_db_clean() call 
...
def MasterConnectionParameters(
    ...
    if not "MasterConnectionParameters" in dir(_eXtremeDB):
        raise Exception ("HA not supported")

    ret = _eXtremeDB.MasterConnectionParameters()
...
def ReplicaConnectionParameters(commitTimeout = None,
    ...
    if not "ReplicaConnectionParameters" in dir(_eXtremeDB):
        raise Exception ("HA not supported")

    ret = _eXtremeDB.ReplicaConnectionParameters()
...
def ReplicaConnection(*args):
    if not "ReplicaConnection" in dir(_eXtremeDB):
        raise Exception ("HA not supported")

    return _eXtremeDB.ReplicaConnection(args)


def HACancel(cancelpoint_addr, timeout):
    return _eXtremeDB.HACancel(cancelpoint_addr, timeout)
