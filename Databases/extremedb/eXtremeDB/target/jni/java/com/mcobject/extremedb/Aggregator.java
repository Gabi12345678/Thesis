 ...
 *         public int   volume;
 *     };
 *     // Query: select  standard deviation of difference between low and high prices for IBM for each month since 1990
 ...
 *         Map<Object,Aggregator.Aggregate> result = Aggregator.<Quote>aggregate(cursor, new Aggregator.GroupBy<Quote>() {
 ...
 *             public Aggregator.FilterResult filter(Quote quote) { return Filter.Use; }
 *         }, true);
...
public class Aggregator
{
     ...
     * @return map with results of aggregation: &lt;group-by,aggregate-value&gr; pairs
     */
    ...
    { 
        ...
        while (iterator.hasNext()) { 
            T obj = iterator.next();
            FilterResult filter = groupBy.filter(obj);
            if (filter == FilterResult.Use) { 
            ...
            } else if (filter != FilterResult.Skip) { 
                break;
            }
        }
        return hash;
