CREATE TABLE datapoints(t TIMESTAMP, d0 DOUBLE, d1 DOUBLE, d2 DOUBLE, d3 DOUBLE);
CREATE TABLE missing_values(t TIMESTAMP, d0 DOUBLE, d1 DOUBLE, d2 DOUBLE, d3 DOUBLE);
...
create function disk_usage() returns integer in 'python' as '
	import os
	def get_size(start_path = "/home/gabi/Databases/extremedb/recovdb"):
		total_size = 0
...
create function import_data() RETURNS string in 'python' as '
	...
	f = open("/home/gabi/Datasets/real.txt.csv", "r")
...
-- import datapoints /home/gabi/Datasets/real.txt.csv;
SELECT import_data();
INSERT INTO metrics_insert VALUES (current_time(), disk_usage());
SELECT  MAX(disk_usage) - MIN(disk_usage) AS disk_usage,
        MAX(current_time) - MIN(current_time) AS insert_time,
        10.0 / (MAX(current_time) - MIN(current_time)) as Throughput_inserts_per_second,
        10.0 * 4.0 / (MAX(current_time) - MIN(current_time)) as Throughput_values_per_second
FROM metrics_insert;
