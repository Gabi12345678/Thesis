...
parser = argparse.ArgumentParser(description = 'Script to run DSTree in Graphite')
...
args.logpath = os.path.abspath("../for-install/log/")
...
for lines in args.lines:
	for columns in args.columns:
		...
		f = open(args.file, "r")
		g = open(args.logpath + "/data.txt", "w")
		t1 = 10**12
		t2 = -1
		index_lines = []
		values = [[]] * lines
		for i in tqdm(range(lines)):
			...
			index_lines.append( values[i][ (columns + 1) : (2 * columns + 1) ] )
		index_lines = np.array(index_lines).T.tolist()
		for i in tqdm(range(columns)):
			g.write(" ".join(index_lines[i]) + "\n")
		g.close()
		f.close()
		...
		params = {
			...
			'until': t2 + 20,
			'format': 'csv',
			'target': 'dstreeindex(master.fake*,' + str(columns) + ')',
		}
		...
		print("+" * 100)
		print("Index time: ", final_time - initial_time)
		print("+" * 100)
		...
		params = {
			...
			'until': t2 + 20,
			'format': 'csv',
			'target': 'dstreesearch(master.' + args.format + '.dim*,' + str(columns) + ')'
		}
		r = requests.get(url = "http://localhost/render", params = params)
		if not(r.ok):
			print(r.text)
		final_time = (datetime.now() - datetime(1970, 1, 1)).total_seconds()
		print("+" * 100)
		print("Search time: ", final_time - initial_time)
		print("+" * 100)
