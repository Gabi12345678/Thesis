...
parser = argparse.ArgumentParser(description = 'Script to run HotSax in Graphite')
parser.add_argument('--file', nargs='?', type=str, help='path to the dataset file', default='../../../Datasets/synthetic.txt')
parser.add_argument('--lines', nargs='*', type=int, default = [400],
...
for lines in args.lines:
	for columns in args.columns:
		...
		params = {
			...
			'until': t2 + 20,
			'format': 'csv',
			'target': 'hotsax(master.' + args.format + '.dim*)',
		}
