...
parser = argparse.ArgumentParser(description = 'Script to run Knn in Graphite')
...
for lines in args.lines:
	for columns in args.columns:
		...
		for i in tqdm(range(lines)):
			...
			labelValues[i] = read_lines[i + lines][:-1].split()
		...
		for j in tqdm(range(columns)):
			for i in range(lines):
				...
				currentTime = (currentTime - datetime(1970, 1, 1)).total_seconds()
				currentTime = int(currentTime)
				insert_value = "master." + args.format + ".ldim" + str(j) + " " + labelValues[i][j + 1] + " " + str(currentTime) + "\n"
				sock.sendall( bytes(insert_value, "UTF-8") )
				if j == 0:
					insert_value = "master." + args.format + ".label " + str(random.randrange(5)) + " " + str(currentTime) + "\n"
					sock.sendall( bytes(insert_value, "UTF-8") )
			if (j + 1) % bucket_size == 0 or j == columns - 1:
				while True:
					try:
		...
		params = {
			...
			'until': t2 + 20,
			'format': 'csv',
			'target': 'knn(master.' + args.format + '.ldim*,master.' + args.format + '.label,master.' + args.format + '.dim*)',
		}
