...
for lines in args.lines:
	for columns in args.columns:
		...
		interval_t1 = int( (datetime.strptime(args.start_time, '%Y-%m-%dT%H:%M') - datetime(1970, 1, 1)).total_seconds() )
		interval_t2 = int( (datetime.strptime(args.end_time, '%Y-%m-%dT%H:%M') - datetime(1970, 1, 1)).total_seconds() )
		params = {
			'from': interval_t1,
			'until': interval_t2,
			'format': 'csv',
			'target': 'master.' + args.format + '.dim*',
		}
