...
for lines in args.lines:
	for columns in args.columns:
		...
		params = {
			...
			'format': 'csv',
			'target': [
				'smartSummarize(master.' + args.format + '.dim*,"10y","average")',
				'smartSummarize(master.' + args.format + '.dim*,"10y","stddev")',
			]
		}
		r = requests.get(url = "http://localhost/render", params = params)
		averages = [0.0] * columns
		stddevs = [0.0] * columns
		l = r.text.split("\n")
		for i in range(columns):
			averages[i] = float(l[i].split(",")[-1])
		for i in range(columns):
			stddevs[i] = float(l[i + columns].split(",")[-1])
		targets = [""] * columns
		for i in range(columns):
			targets[i] = "scale(offset(master.{0}.dim{1},{2}),{3})".format(args.format, i, -averages[i], 1.0 / stddevs[i])
		params = {
			'from': t1 - 20,
			'until': t2 + 20,
			'format': 'csv',
