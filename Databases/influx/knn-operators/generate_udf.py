...
args.cartesian_handler_template_path = os.path.abspath('template_cartesian_handler.py')
args.cartesian_handler_path = os.path.abspath('cartesian_handler.py')
...
args.udf_tick_template = os.path.abspath('template_udf.tick')
args.udf_tick = os.path.abspath('udf.tick')
...
generate_from_template(template_path = args.kapacitor_config_template_path, file_path = args.kapacitor_config_path,
                        changes = [("<cartesian_handler_path>", args.cartesian_handler_path), ("<knn_handler_path>", args.knn_handler_path), ("<home_path>", args.home_path)])
generate_from_template(template_path = args.cartesian_handler_template_path, file_path = args.cartesian_handler_path,
...
generate_from_template(template_path = args.knn_handler_template_path, file_path = args.knn_handler_path,
			changes = [("<kapacitor_library>", args.kapacitor_library)])
...
for line in args.lines:
	for column in args.columns:
		...
		distance_format = "pow(\"l{0}\" - \"d{0}\", 2.0)"
		distance_exp = distance_format.format(0)
		for i in range(1, column):
			distance_exp = distance_exp + " + " + distance_format.format(i)
		generate_from_template(template_path = args.udf_tick_template, file_path = args.udf_tick,
			changes = [("<distance_exp>", distance_exp)])
