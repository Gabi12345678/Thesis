...
# Arguments
parser = argparse.ArgumentParser(description = 'Script to run HotSax in Influx')
...
args.hotsax_handler_template_path = os.path.abspath('template_hotsax_handler.py')
args.hotsax_handler_path = os.path.abspath('hotsax_handler.py')
args.influxd_path = os.path.abspath('../influxdb-1.7.10-1/usr/bin/influxd')
args.tick_path = os.path.abspath('udf.tick')
args.implementation_path = os.path.abspath('../../../Algorithms/hot_sax/')
...
generate_from_template(template_path = args.kapacitor_config_template_path, file_path = args.kapacitor_config_path,
                        changes = [("<handler_path>", args.hotsax_handler_path), ("<home_path>", args.home_path)])
generate_from_template(template_path = args.hotsax_handler_template_path, file_path = args.hotsax_handler_path,
...
for line in args.lines:
	for column in args.columns:
		...
		print("Insert time (seconds): ", seconds)
...
subprocess.run(["rm", args.hotsax_handler_path])
