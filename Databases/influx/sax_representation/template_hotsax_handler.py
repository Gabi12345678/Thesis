...
class HotSaxHandler(Handler):
  ...
  def end_batch(self, end_req):
    ...
    rep = saxtransformation.saxrepresentation(self.matrix)
    end_time = datetime.now()
    t = (end_time - start_time).total_seconds()
    response = udf_pb2.Response()
    response.point.fieldsDouble["SaxTime"] = t
    ...
    for i in range(len(rep)):
      value = rep[i]
      response = udf_pb2.Response()
      response.point.fieldsString["value"] = value
      self.agent.write_response(response)
...
if __name__ == '__main__':
  ...
  logger.info('Starting agent for SaxRepresentation using Sax')
  agent.start()
  agent.wait()
