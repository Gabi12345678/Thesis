...
sys.path.append("/home/gabi/Thesis-master/Algorithms/znormalization")
...
class ZScoreHandler(Handler):
  ...
  def end_batch(self, end_req):
    ...
    zscore = znormalization.zscore(self.matrix)
    end_time = datetime.now()
    t = (end_time - start_time).total_seconds()
    response = udf_pb2.Response()
    response.point.fieldsDouble["ZScoretime"] = t
    #seting timestamp for zscore time
    ...
    output_file = open("/home/gabi/Thesis-master/Databases/influx/zscore/time.txt", "a")
    output_file.write(str(self.matrix.shape) + " -- " + str((self.final_time - self.initial_time).total_seconds()) + "\n")

if __name__ == '__main__':
  agent = Agent()
  handler = ZScoreHandler(agent)
  agent.handler = handler

  logger.info('Starting agent for ZScore')
  agent.start()
  agent.wait()
  logger.info('ZScore agent finished')
