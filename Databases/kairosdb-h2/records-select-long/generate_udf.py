...
for lines in args.lines:
	for columns in args.columns:
		...
		interval_t1 = int( (datetime.strptime(args.start_time, '%Y-%m-%dT%H:%M') - datetime(1970, 1, 1)).total_seconds() ) * 1000
		interval_t2 = int( (datetime.strptime(args.end_time, '%Y-%m-%dT%H:%M') - datetime(1970, 1, 1)).total_seconds() ) * 1000
		dataSave = {
		...
		print("IntervalSelect time:", final_time_udf - initial_time_udf)
		print("*" * 100)
