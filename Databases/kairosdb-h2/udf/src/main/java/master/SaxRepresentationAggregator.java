...
@FeatureComponent(
	...
	description = "UDF for SaxRepresentation"
)
public class SaxRepresentationAggregator implements Aggregator {
	public static final Logger logger = (Logger) LoggerFactory.getLogger(SaxRepresentationAggregator.class);
	...
	@Inject
	public SaxRepresentationAggregator(DoubleDataPointFactory dataPointFactory) {
	...
	public DataPointGroup aggregate(DataPointGroup dataPointGroup) {
			...
			List<String> representation = SaxTransformation.saxrepresentation(data);
		 
			double[][] result = new double[representation.get(0).length()][representation.size()];
			for (int i = 0; i < representation.size(); ++i) {
				for (int j = 0; j < representation.get(i).length(); ++j) {
					result[j][i] = (double) representation.get(i).charAt(j);
				}
			}
			
			logger.info("Applied SaxRepresentation");
