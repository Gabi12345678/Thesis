...
for lines in args.lines:
        for columns in args.columns:
                ...
                for i in tqdm(range(lines)):
                        ...
                        label_values = read_lines[i + lines][:-1].split(" ")[1:columns + 1]
                        values = values + label_values + [str(random.randrange(5))]
                        g.write(",".join(values) + "\n")
                f.close()
                g.close()
                ...
                for i in range(0, columns):
                        column_types = column_types + ", l" + str(i) + " DOUBLE PRECISION"
                unlabel_column_names = "d0"
                for i in range(1, columns):
                        unlabel_column_names = unlabel_column_names + ", d" + str(i)
                label_column_names = "l0"
                for i in range(1, columns):
                        label_column_names = label_column_names + ", l" + str(i)
