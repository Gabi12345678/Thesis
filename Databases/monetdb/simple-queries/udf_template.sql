...
INSERT INTO result_interval SELECT time, d FROM datapoints WHERE time > str_to_timestamp('<start_time>', '%Y-%m-%d') and time < str_to_timestamp('<end_time>', '%Y-%m-%d');
...
INSERT INTO result_moving SELECT time, AVG(d) OVER (ORDER BY time ASC RANGE BETWEEN INTERVAL '<moving_average_hours>' HOUR PRECEDING AND INTERVAL '0' HOUR FOLLOWING) FROM datapoints;
