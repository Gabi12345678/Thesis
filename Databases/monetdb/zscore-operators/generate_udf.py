...
parser = argparse.ArgumentParser(description = 'Script to run Z-Score in MonetDB')
...
args.output_path = os.path.abspath('output_zscore.csv')
...
for lines in args.lines:
        for columns in args.columns:
                ...
                select_format = "(d{0} - (SELECT avg(d{0}) FROM datapoints)) / (SELECT sys.stddev_pop(d{0}) FROM datapoints)"
                select_expression = "\t" + select_format.format(0)
                for i in range(1, columns):
                        select_expression = select_expression + ",\n\t" + select_format.format(i)
