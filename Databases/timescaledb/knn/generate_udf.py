...
parser = argparse.ArgumentParser(description = 'Script to run KNN in TimescaleDB')
...
for lines in args.lines:
	for columns in args.columns:
		...
		h = open(args.file + "_label.csv", "w")
		for i in tqdm(range(lines)):
			values = f.readline()[:-1].split(" ")
			g.write(values[0] + ",\"{" + ",".join(values[1:(columns + 1)]) + "}\"\n")
		for i in tqdm(range(lines)):
			values = f.readline()[:-1].split(" ")
			h.write(values[0] + ",\"{" + ",".join(values[1:(columns + 1)]) + "}\",\"" + str(random.randrange(5)) + "\"\n")
		f.close()
		g.close()
		h.close()
		...
		for l in f.readlines():
			g.write( l.replace("<lines>", str(lines))
				...
				.replace("<label_data_file>", args.file + "_label.csv") )
		f.close()
		g.close()
