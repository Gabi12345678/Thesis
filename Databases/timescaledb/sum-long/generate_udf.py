...
for lines in args.lines:
	for columns in args.columns:
		...
		sum_expr = ",".join(["SUM(d{0})".format(i) for i in range(columns)])
		f = open(args.udf_template, "r")
		g = open(args.udf_template + ".sql", "w")
		for l in f.readlines():
			g.write( l.replace("<column_types>", column_types).replace("<lines>", str(lines)).replace("<columns>", str(columns)).replace("<sum_expr>", sum_expr).replace("<data_file>", args.file + ".csv").replace("<implementation_path>", args.implementation_path) )
		f.close()
		g.close()
